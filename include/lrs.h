...
*/
/* LeCroy warns that we must not use 0x0 as an SIB module address.  The SIB
   bus is held down by default, so 0x0 is addressed even when there is no
   bus activity, causing it to receive "spurious transfer requests".      */ 
#define MOD1821 0xe  /* The 1821 must be set to 0xe on the front panel*/

#define SIB(dev,reg)  (0x000 | ((dev<<5) | (reg<<1)))
#define LIST_RD   0x1000
#define LIST_WR   0
#define LIST_DONE 0x2000
/*  Direct memory access to the SMI */
/* 0x0400 sets Direct mode + bypass + enable 
   after being used as in pointer arithmetic for a short.*/
#define SIB_DIRECT(dev,reg)  (0x0400 | (dev<<4) | reg)

/*  Start of SMI DMA interface */
#define DIRECT  0x800
/*  Start of the 1131 List memory */
#define LIST    0x4000
/*  Start of data memory, used by lists? */
#define DATA    0x8000
