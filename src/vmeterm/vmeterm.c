...
*/
   termio = open("/dev/ttyUSB0",O_RDWR);
   if (termio < 3)
     {
       perror("VMEterm");
       printf("Can not open serial line ttyUSB0\n");
       exit(99);
     }
   if (ioctl (termio,TCGETA,&init_tty) == -1)
     {
       perror("VMEterm");
       printf("Can not get ttyUSB0 configuration\n");
       close(termio);
       exit(99);
     }
/*
*   Configure ttyUSB0 for our use.
*/
   ...
   if (ioctl (termio,TCSETA,&vme_tty) == -1)
     {
       perror("VMEterm");
       printf("Can not set ttyUSB configuration\n");
       close(termio);
       exit(99);
     }
