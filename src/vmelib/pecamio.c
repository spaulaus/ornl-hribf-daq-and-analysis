...
*
*    3/9/05     RLV         Modify for UDP to VME
*    3/10/05    RLV         There is only room for 182 CNAFs in a packet
*   10/19/05    RLV         New packet io library, pkt_io_udp
*****************************************************************************/
#include  <stdio.h>
#include  <stdlib.h>
#include  <string.h>
              ...
              int *len,int *stat)
{
   unsigned short *dat16 = data;
            int   *dat24 = data;
            int   i,j,cnt,status;
   struct cnaf_buffer *out_buf = (struct cnaf_buffer *)xbuf.Data;
   struct cnaf_return *ret_buf = (struct cnaf_return *)rbuf.Data;
...
/*  Dispatch on the type of CAMAC operation    */
   ...
   xbuf.DataSize = sizeof(struct cnaf_buffer);
   rbuf.DataSize = sizeof(struct cnaf_buffer);
   switch  (*f & 0x18)
    {
/*
