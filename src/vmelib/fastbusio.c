...
void fastbusio_(int *a,int *f,unsigned short *data,int *len,int *stat)
{
   ...
   struct fast_buffer *out_buf = (struct fast_buffer *)xbuf.Data;
   struct fast_return *ret_buf = (struct fast_return *)rbuf.Data;
...
/*   Check call parameters for valid values    */
   ...
   xbuf.DataSize = sizeof(struct fast_buffer);
   rbuf.DataSize = sizeof(struct fast_buffer);
   if (*f == 0)
    {
/*
*   Read function 
      ...
      while (cnt > 0)
       {
         ...
         if (i == MAX_FAST || cnt == 0)
           {
             out_buf->count = i;
             status = pkt_io(&xbuf,&rbuf,FASTBUS,1);
             if (status) break;
             j = 0;
             while (j < i)
              {
     ...
     if (i == MAX_FAST || cnt == 0)
       {
         out_buf->count = i;
         status = statusb;
         estatus = pkt_io(&xbuf,&rbuf,FASTBUS,1);
         if(estatus)
          {
