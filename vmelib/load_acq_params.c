...
******************************************************************************
*
*    Environment:  VME Acquisition for Linux/RTEMS
*
*    File:         /tera/mcsq/Drtems/Dacq/load_acq_params.c
*
...
*
*    5/11/05    MCSQ       RTEMS version.
*****************************************************************************/
#include <stdio.h>
#include <stdlib.h>
#include <stddef.h>
#include <string.h>
#include "../include/pkt_io_udp.h"
...
/*   Global variables           */
static struct UDP_Packet xbuf,rbuf;
static char                 server[8] = "vme";
static union Cmd *          inbuf = (union Cmd *)rbuf.Data;
static union Cmd *          outbuf = (union Cmd *)xbuf.Data;
   ...
   if (size >= buflen)
     {
       *buf_ptr = '\0';
       rbuf.DataSize = 0;
/**********
       xbuf.DataSize = sizeof(struct Load)-buflen+1;
       xbuf.DataSize = sizeof(struct Load);
**********/
       xbuf.DataSize = MAX_UDP_DATA;
       xbuf.DataSize = sizeof(struct Load);
       status = pkt_io(&xbuf,&rbuf,CODE,1);
...
#define  SR_BYTES  200     /* Maximum data bytes/S record  */
