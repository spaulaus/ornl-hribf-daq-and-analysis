...
*
*    File:         /usr/users/mcsq/Dlinux/Dacq/pkt_io.c
*
*    Description:
*
******************************************************************************
...
*
*****************************************************************************/
int  pkt_io(struct Vmed *out,struct Vmed *in,int proto,int tmo)
{
   int status;

   if(sockfd == -1)
    {
      pkt_open(out,in,proto,tmo);
    }
    if (out->len < 0 || out->len > initvme.cmd_len)
     {
       printf("pkt_io: Ether transmit buffer size %i out_of_range (limit %i)\n",
               out->len,initvme.cmd_len);
       exit(99);
     }
   if (in->len < 0 || in->len > initvme.rpy_len)
     {
       printf("pkt_io: Ether receive buffer size %i out_of_range (limit %i)\n",
               in->len, initvme.rpy_len);
       exit(99);
     }
   out->error = 0;
...
/******************************************************************************
******************************************************************************/
int pkt_open(struct Vmed *out,struct Vmed *in,int proto,int tmo)
{
   int status,hdr_len;

   /*
   out->len = sizeof(out->buf);
   in->len  = sizeof(in->buf);
   */
   if (out->len < 0 || out->len > MAX_ORPH_DATA)
     {
       printf("pkt_open: Ether transmit buffer size %i bytes out_of_range\n",
	      out->len);
       exit(99);
     }
   if (in->len < 0 || in->len > MAX_ORPH_DATA)
     {
       printf("pkt_open: Ether receive buffer size %i bytes out_of_range\n",
	      out->len);
       exit(99);
     }
   if (proto < DATA || proto > RMSSIO)
     {
       printf("pkt_open: Ether protocol %x out_of_range\n", proto);
       exit(99);
     }
   if (tmo < 0) tmo = 0;
...
/*      printf("pkt_open: cmd_len: %i, rpy_len:%i\n",
	     initvme.cmd_len, initvme.rpy_len);
*/
/******************
printf("Proto = %x\n",initvme.proto);
printf("Timeout = %x\n",initvme.timeout);
...
/******************************************************************************
******************************************************************************/
int pkt_send(struct Vmed *out)
{
   int status;

   if (out->len < 0 || out->len > initvme.cmd_len)
     {
       printf("pkt_send: Ether transmit buffer size out_of_range\n");
       exit(99);
     }
   out->error = 0;
   status = writen(sockfd,(unsigned char *)out,initvme.cmd_len);
...
/******************************************************************************
******************************************************************************/
int pkt_recv(struct Vmed *in)
{
   int status;

   if (in->len < 0 || in->len > initvme.rpy_len)
     {
       printf("pkt_recv: Ether receive buffer size out_of_range\n");
       exit(99);
     }
   status = readn(sockfd,(unsigned char *)in,initvme.rpy_len);
   if (status <= 0)
