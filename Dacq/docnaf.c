...
int main(int argc, char *argv[])  /*  CNAF.C  */
{
   ...
   struct cnaf_buffer *bcnaf = (struct cnaf_buffer *)xbuf.Data;
   struct cnaf_return *ret_buffer = (struct cnaf_return *)rbuf.Data;

   char inps[LINELENGTH]; /* input of CNAFs */
   char error_inps[LINELENGTH];
...
/*-------  Send the list and wait for reply  --------------*/
   ...
   tmo = 2;       /* 2 seconds, retry is 5 times*/
   xbuf.DataSize = cnaflen(bcnaf);
   rbuf.DataSize = cnaflen(bcnaf);
