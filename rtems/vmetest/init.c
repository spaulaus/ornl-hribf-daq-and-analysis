...
extern rtems_task testvme (rtems_task_argument ignored);
...
{
  rtems_id tidcan,tidvme,tiddev,tidres,tidacq,tidtest;
...
/* recommended startup stuff from RTEMS */
  rtems_bsdnet_config.ifconfig->name = RTEMS_BSP_NETWORK_DRIVER_NAME;
  rtems_bsdnet_config.ifconfig->attach = RTEMS_BSP_NETWORK_DRIVER_ATTACH;
...
*/
  ...
  sc = rtems_task_create (rtems_build_name ('V', 't', 's', 't'),
       my_priority,
       32*1024,
       RTEMS_PREEMPT|RTEMS_NO_TIMESLICE|RTEMS_NO_ASR|RTEMS_INTERRUPT_LEVEL(0),
       RTEMS_NO_FLOATING_POINT|RTEMS_LOCAL,
  ...
  sc = rtems_task_start (tidtest, testvme, 0);
  if (sc != RTEMS_SUCCESSFUL) {
    printf ("Can't start task; %s\n", rtems_status_text (sc));
    return;
  }
  else
    printf("testvme started on VME!\n");
 ...
 */
  ...
  for (;;) {
    ...
    default:
      printf("Enter \n");
      printf("q - reboot\n");
      printf("s - network stats\n");
      printf("d - print debug array\n");
      printf("z - zero debug array\n");
      break;
    }
  }
