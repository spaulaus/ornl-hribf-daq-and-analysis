...
Revisions:
  ...
  11/14/93  MCSQ  New shared memory buffer scheme.
   ...
   8/??/03  MCSQ  ported to Linux

   7/14/03  MCSQ  Fixed some error exits to report process name.
========================================================================*/
/*    Include files  */
#include <stdio.h>
#include <stdlib.h>
...
static struct orphmsg msg_pfipc={MSG_INFORM,"rmpftoipc "};

struct orpas_data *orbuf;
       char mess[78];
/*===================================================================
   Main routine of rmpftoipc - copies data from packetfilter to 
   an IPC channel
====================================================================*/
...
/*    Print help if requested  */
   if (argc > 1 && !strcmp(argv[1], "-?")) {
      printf("Usage: rmpftoipc [-d nodename]\n");
      return 1;
   }
   server = getenv("VME");
...
*/
   ...
   if (status != 0)
     {
       perror("\n\7StartTiming");
       fputs(" rmpftoipc - Data rates will NOT be logged.\7\n",stderr);
      }
...
/*   Get the server name. i.e. Front-end Ethernet address  */
   ...
   if (status == FAILURE) {
      fputs("\n\7rmpftoipc - Failed to open Packet Filter Interface\n",stderr);
      fputs("rmpftoipc - TERMINATED\7\n",stderr);
      return 1;
   }
...
/*   Get the IPC file name to use.                     */
   ...
   if (status == FAILURE) {
      fputs("\n\7rmpftoipc - Failed to open IPC Interface\n",stderr);
      fputs("rmpftoipc - TERMINATED\7\n",stderr);
      return 1;
   }
   buffers_init();
...
*/
       ...
       if (status==FAILURE) {
           fputs("\n\7rmpftoipc - Error reading from Front-end\n",stderr);
           break;
       }
       packets++;
       TotalBytes=TotalBytes+retlen-DATAHDRSZ;   /* size of header */

       orbuf = (struct orpas_data *)pkt_buf->buf;
...
*/
       ...
       if (msgsnd(Ids.log_msg, &msg_pfipc, sizeof(struct orphmsg), IPC_NOWAIT) != 0)
         {
           fputs("\n\7rmpftoipc - Error writing Message Queue\n",stderr);
           TermHandle(0);
         }
       sprintf(&msg_pfipc.text[29],
...
/*=====================================================================
   ...
   terminate the rmpftoipc program
 =====================================================================*/
...
void TermHandle (int sig)
{
   ...
   time(&tod);
   strftime(&msg_pfipc.text[9],21,"%d-%b-%y %H:%M:%S  ",localtime(&tod));
   strcpy(&msg_pfipc.text[29],"rmpftoipc EXITING!");
...
/*======================================================================
  ...
  isolate the Packet Filter routines from the main body of the rmpftoipc
  routine.
=======================================================================*/
...
int PFSetup(char *pfserver)
{
      ...
      pkt_data.len = sizeof(pkt_data.buf);
      xmit.len = 0;
      pfdata= pkt_open(&xmit, &pkt_data,  DATA, 60);
      pkt_send(&xmit);
...
int data_recv(struct Vmed *buffer,int *buflen)
{
    int len;
    ...
    *buflen = 0;
    while (*buflen == 0)
     {
       readn(pfdata, (unsigned char *)buffer, sizeof(struct Vmed));
       *buflen = buffer->len;
     }
    /*   return  with info.*/
    return SUCCESS;
...
int IPCSetup(char *ipcnam)
{
  ...
  open_acq_ipc__(ipcnam,&size,&ierr,strlen(ipcnam));
  if (ierr != 0)
    {
