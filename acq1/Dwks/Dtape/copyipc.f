...
*                  bean counters.
* 01Oct08    RLV.  Further add time elapsed since trun to message
*******************************************************************************
      subroutine copyipc(kmd,nfnf,ierr)
*
      implicit none
*
      include  "../../Dacq/acq_vme_ctrl.for"
*
      integer*4       nfnf,ierr
      character*4 kmd
...
*     Get status from the argument to TRUN
      integer beamstat
*     Save the start time, compute the elapsed time
      integer starttod, stoptod, elapsedtod
      save starttod, stoptod, elapsedtod
*
...
*
      ...
      if (vstat .lt. 0) then
        write(cmssg,*) 'VME Ethernet Connection Failure'
...
555     format('RECORDS-THIS-FILE, TOTAL-RECORDS COPIED =',2I8)
        stoptod = time()
        elapsedtod = stoptod - starttod
        call messlog(logut,logup)
        call doecntlog(mssg,beamstat,sumevts,nrtf,elapsedtod)
        call messlog(logut,logup)
