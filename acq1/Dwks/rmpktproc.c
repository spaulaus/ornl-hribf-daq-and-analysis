...
/*  Prototypes of functions in pftoipc.c   */
...
int max_buf;
char savbuf[sizeof(struct Vmed_hdr)+DATAHDRSZ];

struct Vmed *pktprocess(struct Vmed *pkt_buf,int buflen)
{
...
*/
   en_buf = (struct orpas_data *)pkt_buf->buf;
       ...
       if (expcont)
         {
           ...
           used = contused;
           expcont = 0;
           pkt_buf = (struct Vmed *)(datptr - DATAHDRSZ - sizeof(struct Vmed_hdr));
           memcpy(savbuf,(char *)pkt_buf,DATAHDRSZ+sizeof(struct Vmed_hdr));
         }
       return pkt_buf;
     }

   memcpy(pkt_buf,savbuf,DATAHDRSZ+sizeof(struct Vmed_hdr));
...
*/
       ...
       cptr = datptr - DATAHDRSZ - sizeof(struct Vmed_hdr);
       memcpy(savbuf,cptr,DATAHDRSZ+sizeof(struct Vmed_hdr));
       if (thispkt == LCONT) expcont = 0;
       return  (struct Vmed *)cptr;
     }
   if (expcont == 0)
     {
/*
...
void buffers_init(void)
{
   ...
   nextbuf->events = 0;
   pkt_buf = (struct Vmed *)(((char *)curbuf->data)-DATAHDRSZ-sizeof(struct Vmed_hdr));
   memcpy(savbuf,pkt_buf,DATAHDRSZ+sizeof(struct Vmed_hdr));
}
