...
int max_buf;
char savbuf[ORPH_HDR_LEN+DATAHDRSZ];

struct Ether_Packet *pktprocess(struct Ether_Packet *pkt_buf,int buflen)
{
       ...
       if (expcont)
         {
           ...
           used = contused;
           expcont = 0;
           pkt_buf = (struct Ether_Packet *)(datptr - DATAHDRSZ - ORPH_HDR_LEN);
           memcpy(savbuf,(char *)pkt_buf,DATAHDRSZ+ORPH_HDR_LEN);
         }
       return pkt_buf;
     }

   memcpy(pkt_buf,savbuf,DATAHDRSZ+ORPH_HDR_LEN);
...
*/
       ...
       cptr = datptr - DATAHDRSZ - ORPH_HDR_LEN;
       memcpy(savbuf,cptr,DATAHDRSZ+ORPH_HDR_LEN);
       if (thispkt == LCONT) expcont = 0;
       return  (struct Ether_Packet *)cptr;
     }
   if (expcont == 0)
     {
/*
...
void buffers_init(void)
{
   ...
   extern struct Ether_Packet *pkt_buf;
   ...
   nextbuf->events = 0;
   pkt_buf = (struct Ether_Packet *)(((char *)curbuf->data)-DATAHDRSZ-ORPH_HDR_LEN);
   memcpy(savbuf,pkt_buf,DATAHDRSZ+ORPH_HDR_LEN);
}
