...
*
*    File:         /usr/users/mcsq/Dvme3/vme_boot60.s
*
...
*
*    3/ 2/97   MCSQ         Modified 'vme_boot.s' to work with new CPU-60s.
*                           Changes due to difference in the ethernet
*                           controller chips for the CPU-40 and CPU-60.
*                           Also interrupt level and setup are different.
*
*                           Added code to get board ethernet address by
*                           calling a function is the FGA boot PROM.
*                           The same loader should work for all CPU-60s.
*
*    3/ 7/97   MCSQ         Disable CTS handshake for serial I/O ports
*                           1 and 2.
*******************************************************************************
*
	ORG     $FFC08000
...
LAN_VEC                 EQU     247             ;Am79C965 LAN vector
LAN_CTL                 EQU     $FFD002A8
LAN_STAT                EQU     $FFD0049C

RINT                    EQU     $400            ;Receive flag

CPU40_BOOT              EQU     $FFD00E00
FGA_BOOT		EQU	$FFE00000

PORT_FLAGS		EQU	$1048		;I/O port flag bits in
*						;SYRAM.  char _f8bt[16].
*
		INCLUDE vmeprom.inc
*
...
BOOT_GO:        
		DC.W	$A02C		;set supervisor mode
		MOVE.L	#LAN_ADDR,-(SP)
		CLR.L	-(SP)
		MOVE.L	#40,-(SP)
		MOVE.L	#FGA_BOOT+8,A1
		MOVE.L	(A1),A1
		JSR	(A1)		;Get Ethernet address
...
STRUP           MOVE.B  (A0)+,(A1)+     ;Move code
		...
		MOVE.L	#PORT_FLAGS,A0
		ADDQ.W	#1,A0		;Select serial port 1
		MOVE.B	#$0B,(A0)+	;Disable CTS check for port 1
		MOVE.B	#$0B,(A0)+	;Disable CTS check for port 2
...
IPL:
                LOCK
                BSR.W   LAN_INIT        ;Initialize Am79C965 LAN controller
                LEA     MESSG(PC),A1    ;Output start-up message to console
                MOVE.W  (A1)+,D7
                OUT_MSG
                UNLOCK
*
MAIN		DLY_SET_EVT     1,1000  ;Set event #1 after 10 seconds
...
IPL_MES:
	DC.B	0,0,0,0,0,0
	DC.B    'V3.0 IPL Force CPU-60 : Ready for download',CR,0,$FF
*
...
*   Ethernet parameters
*
LAN_ADDR        DC.B    $00,$00,$00,$00,$00,$00         ;Our address
...
****************************************************************************
		...
		ADD.W	2(A1),D0	;That host is now our default
		ADD.W	#6,D0		;host.
		MOVE.L	D0,A1
...
LAN_INIT        MOVE.L	#LANC,A3        ;Am79C965 LAN chip base address
		...
		MOVE.W	RST(A3),D0	;Reset Am79C965 controller
...
LAN_INIT11      MOVE.W  RDP(A3),D1      ;Wait till done
		ANDI.W  #$100,D1
		BEQ     LAN_INIT11
                MOVE.W  #3,(A3)		;Select and load CSR3
		...
		RTS
*******************************************************************************
*  Return Am79C965 LAN controller status in D0
*******************************************************************************
...
LAN_READ1       MOVE.L  #LAN_RAM,D0     ;Calculate address of packet data
		...
		CLR.L	D0
		MOVE.B  (A0),D0         ;Get receive buffer status
		MOVE.W  D0,4(SP)
...
INTR_SETUP:     MOVE.L  RBUF_PTR,A0             ;Release the last received
		MOVE.B  #$80,(A0)               ;packet and update pointers.
		ADD.W   #8,A0
		CMP.L   RBUF_PTR_END,A0
		BNE.S   INTR_SETUP1
		MOVE.L  RBUF_PTR_STR,A0
INTR_SETUP1     MOVE.L  A0,RBUF_PTR
		MOVE.L  #LANC,A0
		MOVE.W	#2,(A0)			;Set INTLEVEL in BCR2
		CLR.W	BDP(A0)			;to zero
		MOVE.L  #LAN_INTR,LAN_VEC*4	;Set interrupt vector
		MOVE.B  #$2F,LAN_CTL            ;Enable local 7 at level 7
		MOVE.W	#0,(A0)
		MOVE.W  #$40,RDP(A3)            ;Enable Am79C965 LAN interrupts.
		RTS
*
...
LAN_INTR:       MOVEM.L D0/A0-A1,-(SP)
		...
		CLR.W   (A1)            ;Select CSR0
		MOVE.W  RDP(A1),D0      ;Get status
...
*
LAN_INTR1:
		MOVE.B  #$80,(A0)       ;Release packet buffer and update
		ADD.W   #8,A0           ;pointers.
		CMP.L   RBUF_PTR_END,A0
		BLT.S   LAN_INTR2
		MOVE.L  RBUF_PTR_STR,A0
