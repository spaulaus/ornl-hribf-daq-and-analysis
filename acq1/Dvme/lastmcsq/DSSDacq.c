...
*                          2277 is the only module known to meet this criterion.*                          JWK, Notre Dame. 
*
*
...
*/
   ...
   while(lrs_intf[i] != NULL)
     {
       (lrs_intf[i])->mode = 1;
sprintf(error_msg,"LRS1190 at %x",lrs_intf[i]);
host_message(INFORM,error_msg);
       i++;
     }
...
int XIA_format(void)
{
  ...
  register unsigned short *dat,*dat_end;
  register unsigned short mod;
  register int cnt,cntst,cntm, vsndgf;
  register unsigned short *evt = Event;
  struct KSC_VME *ksc = (struct KSC_VME *)KSC1;
...
*
*/
	  hdr = *dat;    /*number of counts in on xia buffer*/
	  dat += 2;
	  if (hdr < 0x8000) continue; 
	  ...
	  while(dat < dat_end)
	    {
	      ...
	      dat = dat + 2; /* jport outputs buf_length twice*/
	      cnt = *dat;
	    ...
	    while(cnt--) /* read only one  xia buff at the time.*/
		{
		  *evt++ = *dat;
		  dat += 2;
		}
