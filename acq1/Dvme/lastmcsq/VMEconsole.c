...
*
*    File:         /tera/mcsq/Dlinux/Dvme/VMEconsole.c
*
*    Description:  
*                 
******************************************************************************
...
*/
   ...
   while(1)
   {
     FD_SET(termio,&readfds);
     FD_SET(0,&readfds);
     if ((status=select(4,&readfds,(fd_set *)0,(fd_set *)0,NULL)) < 0)
                                                                   cleanup(99);
     if (status == 0) printf("Timeout\n");
     if (FD_ISSET(0,&readfds))
      {
      if(fgets(tbuf,sizeof(tbuf)-1,stdin) == NULL) cleanup(0);
       {
...
{
   printf("\n");
   printf("break         - Send BREAK to the VME processor\n");
