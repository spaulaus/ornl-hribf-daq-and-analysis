...
*
*    File:         /usr/users/mcsq/Dvme3/VMEacq.c
*
...
*
*    2/08/06    MCSQ       Change so that acquisition interrupt may be
*                          at VME level 5 or VME level 1.
*
*    7/12/06    MCSQ       Add control of SIS3820 VME scaler module.  Changed
*                          acq_start() to enable counting for the scaler.
*                          Changed acq_stop() to disable counting.
*****************************************************************************/
#include "syram.h"
#include "vme_sys.h"
#include "vmeprom.h"
...
struct cnafdat_list         *tmp_camac;
struct cnaf_list            *tmp_cnaf;
struct cond_cam_pgm         *tmp_cond;
struct raw_gate_spec        *tmp_gate;
int                         *tmp_ids;
int                         Faux[18];
...
struct cond_tbl      *Cond_RO_str, *cond_ro, Cond_RO[500];
struct fcond_tbl     *Fcond_RO_str, *fcond_ro, Fcond_RO[500];
struct cam_io        *Cond_cam_str, *cond_cam, Cond_Cam[2000];
struct fcam_io       *Fcond_cam_str, *fcond_cam, Fcond_Cam[500];
struct cam_io        *Uncond_RO_str, *uncond_ro, Uncond_RO[500];
struct fcam_io       *Funcond_RO_str, *funcond_ro, Funcond_RO[500];
unsigned short       Uncond_cma;
unsigned short       *Seq_ID_str, *seq_id, Seq_ID[400];
unsigned short       Seq_cma,seq_transfers;
unsigned short       seq_buf[400];
struct cam_io        *Windup_str, *windup, WindUp[400];
struct fcam_io       *Fwindup_str, *fwindup, FwindUp[400];
unsigned short       Windup_cma;
unsigned short       gate_ro_cma;
struct count_dwn_lst *Count_dwn_str,Count_dwn_tst[50];
struct cam_ro_lst    *Cam_RO_str,*Cam_RO_end,Cam_RO[100];
struct fcam_ro_lst   *Fcam_RO_str,Fcam_RO[100];
struct gate_ro_lst   *Gate_RO_str,Gate_RO[100];
struct cal_gate_lst  *Cal_RO_str,Cal_Gate_RO[100];
unsigned short       start_list_cma1;
unsigned short       start_list_cma2;
int                  xia_flag = 0;
int                  xia_startup_cma,xia_restart_cma,xia_poll;

int               kill_count;
struct kill_list  Kill_List[30];
...
*   Latch_data[1].
*/
unsigned short  Latch_flag;       /* 0 means no latches, 1 means latches */
unsigned short  Latch_cma;
unsigned short  Latch_aux;
unsigned short  Latch_data[50];
unsigned short  Latch_ro[50];
struct fcam_io  Flatch_ro[50];
unsigned short  Fast_id[65536];
unsigned short  Fera_id[8192];
unsigned short  Fera_types[256];
unsigned short  Cam_id[8192];
unsigned short  Caen_adc_id[340];
unsigned short  Caen_tdc_id[340];

int vme_enable;
int vme_delay;
struct vme_readout caen_775;
struct vme_readout caen_785;

int fera_enable;
int ces_early_enable;
int fera_delay;
struct fera_readout baklash;
struct fera_readout baklash2;
struct fera_readout baklash3;
struct fera_readout lrs_4300;
/*      Added support for ORTEC AD413 module.   (JJK, Jan. 29, 1997) */
struct fera_readout ad_413;
/*      End of added support for ORTEC AD413 module.   (JJK, Jan. 29, 1997) */
struct fera_readout lrs_3377;
struct fera_readout gan_812f;
struct fera_readout silena_4418;

int fb_delay;
struct fb_readout fb_1885;
struct fb_readout fb_10C6;
struct fb_readout fb_1881M;
struct fb_readout fb_1877; /* RLV added for 1877 */
struct fb_readout fb_1875;

int fb_1885_count;
int fb_10C6_count;
int fb_1881M_count;
int fb_1877_count; /* RLV added for 1877 */
int fb_1875_count;

int cam_special_delay;
struct cam_readout phil_7164;
struct cam_readout phil_7186;
struct cam_readout lrs_2277;
struct cam_readout lrs_3377C;
struct cam_readout lrs_4300C;
/*      Added support for ORTEC AD413 module.   (JJK, Jan. 23, 1997) */
struct cam_readout ad_413C;
/*      End of added support for ORTEC AD413 module.   (JJK, Jan. 23, 1997) */
struct cam_readout silena_4418C;
struct cam_readout silena_4418C;

int cam_Cond_delay;
int cam_Uncond_delay;

int (*camac_seq[10])(void);
int (*fb_seq[10])(void);
int (*fera_seq[10])(void);
int (*kill_seq[10])(void);
...
/*
*    List of pointers to CAEN ADC and CAEN TDC modules
*/

struct caen_readout caen_adcs[11];
struct caen_readout caen_tdcs[11];

/*
*    VME 100Hz clock
*/

unsigned short *clk100 = (unsigned short *)&((struct SYRAM *)SYSRAM)->_clk100;
           int clk100_flg;
unsigned short clk_id_hi;
unsigned short clk_id_low;

/*
*     Data buffers and pointers
*/
unsigned short *Event;
unsigned short *Startevt;
...
void init_readout(void);

int  crates_on_line(void);
void aux_crates(void);
int  set_CAMAC_inhibit(void);
int  remove_CAMAC_inhibit(void);
int  check_CAMAC_inhibit(void);
int  cam_cond_setup(void);
int  cam_cond_setdelay(void);
int  cam_fcond_setup(void);
int  cam_cond_getf(void);
int  cam_cond_getksc(void);

int  cam_cond_opt(int *);
int  prior_ref(int);

int  latch_setup(void);
int  flatch_setup(void);
int  kill_setup(void);
int  windup_setup(void);
int  fwindup_setup(void);
int  cam_uncond_setup(void);
int  cam_funcond_setup(void);
int  init_cam_modules(void);
int  init_fb_modules(void);
int  init_fera_modules(void);
int  init_vme_modules(void);

int  count_down_setup(void);
int  gate_list_setup(void);
int  cam_special_setup(void);
int  cam_fspecial_setup(void);
int  cam_special_setdelay(void);
int init_xia(void);
int check_xia_mod(int,int);
int  setup_cam_start_list(void);
void xia_start(void);
void run_start_list(void);

void fb_exec(int );
void fb_write_pdreg(int );
unsigned char fb_read_pdreg(void);
unsigned short fb_read_reg(int);
void fb_write_reg(int,unsigned short);
int  fb_read32(void);
void fb_write32(int );
int  fb_prim_addr(int ,int );
void fb_sec_addr(int );
void fb_getSS(char *,char );
void fb_reset_seq(void);

int  ksc_camio(struct camac *);
void fblist(struct fastd *fast);
void fastio(struct fastd *);
void host_message(int ,char *);
int  camac_list(unsigned short ,struct cam_io *);
int  clist(struct cnafdat_list *,int);
...
int  run_uncond_list(void);
int  run_funcond_list(void);
int  run_windup_list(void);
int  run_fwindup_list(void);
int  read_list_seq(void);
int  format_list_seq(void);
int  run_cond_list(void);
int  run_fcond_list(void);
int  fb_wait_done(void);
int  fb_format_1885(void);
int  fb_read_1885(void);
int  fb_transfer_1885(void);
int  fb_format_10C6(void);
int  fb_read_10C6(void);
int  fb_transfer_10C6(void);
int  fb_format_1881M(void);
int  fb_read_1881M(void);
int  fb_transfer_1881M(void);
/*      RLV added for 1877     */
int  fb_format_1877(void);
int  fb_read_1877(void);
int  fb_transfer_1877(void);
/*      End of 1877            */
int  fb_format_1875(void);
int  fb_read_1875(void);
int  fb_transfer_1875(void);
int  fera_read(void);
int  ces_fera_format(void);
int  lrs_fera_format(void);
int  vme_format(void);
int  run_kill(void);
int  seq_nop(void);
int  run_count_down(void);
int  run_gate_list(void);
int  run_cam_special(void);
int  run_fcam_special(void);
int  poll_xia(void);
int  XIA_format(void);
...
/* By default, there is a one-to-one mapping of VME interrupts to CPU interrupt
*  levels for 68040s and 68060s.  The MVME5500 uses VME level 1 for the
*  acquisition interrupt.  So, to make the trigger module work in 680x0 
*  systems and MVME5500 systems, map VME level 1 to CPU level 5 in 680x0
*  processors.
*
*  The trigger modules can be changed to use VME level 1 interrupt.
*  Once the jumpers are changed in the trigger module, the modules
*  can be used in 680x0 and MVME5500 systems.
*/
   *((unsigned char *)VME_LVL1) = 0x0d;
   ...
   while(1)
    {
     ...
     lan_reply(len,NOACK);                /* Send command response   */
    }
}
/****************************************************************************
*
...
/*
*   Disable counting for the SIS3820 VME scaler.
*/
...
/*
*   Patch the 68040 event interrupt vector.
*/
       super_mode_();
       trigger->iera = 0;
       trigger->imra = 0;
...
/*
*   Enable counting for the SIS3820 VME scaler.
*/

   if (devtbl->sis3820_1) {sis->Key_enable = 0; sis->op_mode = 1;}
...
struct raw_gate_spec *gspec1,*gspec2;
	...
	static char *table_names[] = {"FILENAME","CRATES","CNAF_INIT",
				      ...
				      "FERA_ID","WINDUP_CNAF","RUN_CNAF_LIST",
                                      "XIA_LIST","CAEN_RO", "CAEN_ADC_HARD",
                                      "CAEN_TDC_HARD", "CAEN_ADC_ID",
                                      "CAEN_TDC_ID", "CLK100" };
		int invalid = 0;
                int sum_len = 0;
...
#ifdef  DEBUG
      sprintf(error_msg,"off = %x, len = %d, dly = %x %s",
                   tblptr->offset,tblptr->length,tblptr->delay,table_names[i]);
      host_message(INFORM,error_msg);
#endif
      tblptr++;
    }
...
*/
   ...
   if (*(cptr+tblptr->length*4-1) != 0xff)
     {
       byte_swap_(cptr,tblptr->length*4);
       word_swap_((short *)cptr,tblptr->length*2);
...
#ifdef DEBUG
   host_message(INFORM,"RunTime FASTBUS ID table **************");
   sprintf(error_msg,"Fast_id = %x",Fast_id);
   host_message(INFORM,error_msg);

   host_message(INFORM,"RunTime FERA ID table **************");
   sprintf(error_msg,"Fera_id = %x",Fera_id);
   host_message(INFORM,error_msg);
#endif
...
#endif
#ifdef  DEBUG
   sprintf(error_msg,"camac seq = %x",camac_seq);
   host_message(INFORM,error_msg);
   sprintf(error_msg,"fb seq = %x",fb_seq);
   host_message(INFORM,error_msg);
   sprintf(error_msg,"Event data 1 = %x, Event data 2 = %x",
                                    Buf1.Data,Buf2.Data);
   host_message(INFORM,error_msg);
...
#ifdef  DEBUG
   host_message(INFORM,"Conditional CAMAC list ***********");
#endif
...
#ifdef  DEBUG
  sprintf(error_msg,"cond_tbl = %x",Cond_RO);
  host_message(INFORM,error_msg);
  sprintf(error_msg,"cond_cam = %x",Cond_Cam);
  host_message(INFORM,error_msg);
  sprintf(error_msg,"Latch_data = %x",Latch_data);
  host_message(INFORM,error_msg);
  tmp_ro = Cond_RO;
  while(tmp_ro < cond_ro)
    {
      ...
      host_message(INFORM,error_msg);
      sprintf(error_msg,"TRUE  - ctr = %x, cma = %x, camid = %x, next = %x",
							tmp_ro->tr_ctr,
							tmp_ro->tr_cma,
							tmp_ro->tr_camid,
							tmp_ro->tr_next);
      ...
      sprintf(error_msg,"FALSE - ctr = %x, cma = %x, camid = %x, next = %x",
							tmp_ro->fa_ctr,
							tmp_ro->fa_cma,
							tmp_ro->fa_camid,
							tmp_ro->fa_next);
...
#ifdef  DEBUG
   host_message(INFORM,"Latch list ***********");
...
#ifdef  DEBUG
   host_message(INFORM,"Windup list ***********");
   clist(naf,tblptr->length);
#endif
...
*/
  ...
  if (windup->dir != CAM_RO_END)
    {
      host_message(INFORM,"Windup list does not end properly");
    }
#endif
...
#ifdef  DEBUG
   host_message(INFORM,"UnConditional CAMAC list ***********");
   clist(naf,count);
#endif
...
*/
   ...
   if (seq_count > (sizeof(Seq_ID)/sizeof(unsigned short)))
     {
       host_message(PANIC,"List Sequencer Buffers too small!");
   ...
   if (seq_count != 0)
     {
       ...
       if ((status & (KSC_CSR_TMO | KSC_CSR_NOX | KSC_CSR_NOQ)) != 0)
         {
           host_message(PANIC,"No KSC3982 List Sequencer Found!");
           set_intr_level_(level);
...
*/
  ...
  if (uncond_ro->dir != CAM_RO_END)
    {
      host_message(INFORM,"Unconditional readout list does not end properly");
    }
#endif
...
#ifdef  DEBUG
   host_message(INFORM,"Initialize CAMAC modules ***********");
...
#ifdef  DEBUG
   host_message(INFORM,"Clear FASTBUS modules ***********");
#endif
...
/*     End of 1877 block                           */
...
#ifdef  DEBUG
   host_message(INFORM,"Initialize FASTBUS readout ***********");
#endif
   ...
   while(count)
	 ...
	 {
           host_message(PANIC,"Unknown FASTBUS module type in readout specs");
           ++prog;
           --count;
           continue;
         }
...
#ifdef  DEBUG
   sprintf(error_msg,"1885 - Enab = %d, lat = %x, mask = %x",
                                      fb_1885.enable,fb_1885.lat,fb_1885.mask);
   host_message(INFORM,error_msg);
   sprintf(error_msg,"10C6 - Enab = %d, lat = %x, mask = %x",
                                      fb_10C6.enable,fb_10C6.lat,fb_10C6.mask);
   host_message(INFORM,error_msg);
   sprintf(error_msg,"1881M - Enab = %d, lat = %x, mask = %x",
                                   fb_1881M.enable,fb_1881M.lat,fb_1881M.mask);
   host_message(INFORM,error_msg);

/*      RLV added for 1877 readout           */
   sprintf(error_msg,"1877 - Enab = %d, lat = %x, mask = %x",
                                      fb_1877.enable,fb_1877.lat,fb_1877.mask);
   host_message(INFORM,error_msg);
/*      End 1877 block                       */

   sprintf(error_msg,"1875 - Enab = %d, lat = %x, mask = %x",
...
#ifdef  DEBUG
   host_message(INFORM,"Clear FERA modules ***********");
#endif
...
/*      End of added support for ORTEC AD413 module.   (JJK,JWK Jun 26, 2000) */
          ...
          case  SILENA_4418:
            ...
            break;
          default:
            host_message(PANIC,"Unknown FERA module type in readout specs");
            ++prog;
            --count;
            continue;
            break;
...
#ifdef  DEBUG
   sprintf(error_msg,"4300 - Enab = %d, lat = %x, mask = %x",
                                   lrs_4300.enable,lrs_4300.lat,lrs_4300.mask);
   host_message(INFORM,error_msg);
/*      Added support for ORTEC AD413 module.   (JJK, Jan. 29, 1997) */
   sprintf(error_msg,"413 - Enab = %d, lat = %x, mask = %x",
                                   ad_413.enable,ad_413.lat,ad_413.mask);
   host_message(INFORM,error_msg);
/*      End of added support for ORTEC AD413 module.   (JJK, Jan. 29, 1997) */
   sprintf(error_msg,"812F - Enab = %d, lat = %x, mask = %x",
                                   gan_812f.enable,gan_812f.lat,gan_812f.mask);
   host_message(INFORM,error_msg);
   sprintf(error_msg,"4418 - Enab = %d, lat = %x, mask = %x",
...
int init_vme_modules(void)
{
  ...
  struct cond_vme_pgm *prog;
   ...
   for (i=0; i < 11; i++) caen_adcs[i].hwd = NULL;
   for (i=0; i < 11; i++) caen_tdcs[i].hwd = NULL;
       ...
       switch (prog->type_code)
        {
          ...
          case  CAEN_785:
            ...
            break;
          default:
            host_message(PANIC,"Unknown VME module type in readout specs");
            ++prog;
            --count;
            continue;
            break;
     ...
     struct raw_gate_spec  *gate_spec;
      struct cal_gate_lst  *cal_ro;
       struct gate_ro_lst  *gate_ro;
                      int  camf;
                      int  count,level,*cal_spec,*last,last_cnaf;
                      int  cma = Current_cma;
   register struct KSC_VME *ksc = (struct KSC_VME *)KSC1;
...
/*   Deleted next two lines and modified third  (1/10/97 nsc, jjk, jk)   */
/*            ksc->cmr = BLOCK(cam_ro->c,Q_Stop,WS24,A_DIS);             */
...
****************************************************************************/
unsigned short fb_read_reg(int reg)
{
   struct fastd fbcmd;
...
****************************************************************************/
void fb_write_reg(int reg,unsigned short dat)
{
   struct fastd fbcmd;
...
****************************************************************************/
int fb_prim_addr(int type,int slot)
{
   int  temp;
...
****************************************************************************/
void fb_getSS(char *fault,char code)
{
   struct fastd fbcmd;
   ...
   sprintf(error_msg,"%s error, Module = %d, NTA = %d, SS = %d",fault,
						fb_module,fb_nta,code,fbcmd.d);
   host_message(PANIC,error_msg);
}
/****************************************************************************
*
...
*/
       case  0x8:
       case  0x18:
	 level = set_intr_level_(0x500);  /* disable acq interrupts */
	 ksc->cma = 0;
	 ksc->cmr = wd0;
	 ksc->cmr = wd1;
	 ksc->cmr = HALT;
...
/****************************************************************************
****************************************************************************/
int  camac_list(unsigned short cma,struct cam_io *cam)
{
...
****************************************************************************/
clist(struct cnafdat_list *naf,int wc)
{
  int  i;
...
#pragma ghs nofloat
   ...
   struct trig *trigger = (struct trig *)TRIGGER;
   struct KSC_VME *ksc = (struct KSC_VME *)KSC1;
   struct acq_share *shr = (struct acq_share *)ACQ_SHARED_RAM;
   int size,status;
...
*   interrupts.  After that the ORNL trigger module timer brings us here.
*   When we can send the message pointer, Event interrupts are reenabled
*   and trigger module timer is disabled.  Event interrupt service is restored
...
*/
   ...
   lrs_cmd_ptr = (struct LRS_VME *)LRS1;
   evt = Event;
   id = Fast_id;
...
int fb_format_10C6(void)
{
   ...
   register unsigned short *id;
/**********
     static unsigned short *tdcdmp = (unsigned short *)0x30000;
**********/
...
/**********
   if (tdcdmp < (unsigned short *)0x3f000)
  {
   dat = (unsigned short *)(LRS1 + DATA);
   dat_end = dat + fb_10C6_count;
   while(dat < dat_end) *tdcdmp++ = *dat++;
   *tdcdmp++ = 0xfefe;
   *tdcdmp++ = 0xfefe;
  }
**********/
...
int fb_format_1881M(void)
{
   ...
   register unsigned short *id;
/**********
     static unsigned short *adcdmp = (unsigned short *)0x30000;
**********/
#ifdef  TIMER
   timer[Tadcform] = *((unsigned char *)ORNL_COUNT);
#endif
...
/**********
   if (adcdmp < (unsigned short *)0x3f000)
  {
   dat = (unsigned short *)(LRS1 + DATA);
   dat_end = dat + fb_1881M_count;
   while(dat < dat_end) *adcdmp++ = *dat++;
   *adcdmp++ = 0xfefe;
   *adcdmp++ = 0xfefe;
  }
**********/
