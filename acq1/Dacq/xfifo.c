...
int  cc[256],nn[256],aa[256],rdf[256],wrf[256],astat[256];
...
*************/
  camlist_(&cc,&nn,&aa,&rdf,&one,&rdat,&nwds,&astat);
 ...
 *************\n",i-1,wdat[i-1],rdat[i-1]);
  ...
  XOR= %6x\n",i,tdat,rdat[i],diff);
      ...
      if (astat[i] != 0) {printf(" Read Status Error - %x\n",astat[i]); exit(99);}
    }
}
...
************/
  ...
  camlist_(&cc,&nn,&aa,&wrf,&one,&wdat,&nwds,&astat);
  for (i=0; i < 256; i++)
    {
     if (astat[i] != 0)
      {
        printf(" Write Status Error - %x\n",astat[i]);
        exit(99);
      }
    }
}
