...
int acq_vme_ctrl_(int* istat,char *for_cmd,int for_len)
{
   ...
   static char *cmd[] = {"init","start","stop","status","zero",NULL};
   static char cmd_code[] = {INIT_ACQ,START_ACQ,STOP_ACQ,STATUS_ACQ,
                             ZERO_CLK,0};
...
*/
   ...
   xbuf.buf[0] = cmd_code[i];
   xbuf.len = 5;
   rbuf.len = 100;
   pkt_close();
   pkt_io(&xbuf,&rbuf,FECNTRL,10);
   pkt_close();
   if (rbuf.error < 0)
     {
       *istat = ETHER_ERROR;
   ...
   if (xbuf.buf[0] == STATUS_ACQ)
     {
       *istat = (char)rbuf.buf[1];
        return ((char)rbuf.buf[1]);
     }
   *istat = (char)rbuf.buf[0];
   return ((char)rbuf.buf[0]);
}
/******************************************************************************
*
