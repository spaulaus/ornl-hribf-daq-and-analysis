...
char caen792_ctl(struct caen_ctl *);
...
char caen775_ctl(struct caen_ctl *cmd)
{
   ...
   if (cmd->cvt_num < 0 || cmd->cvt_num > 11) return(CAEN_ERR_NONEXIST);
...
char caen792_ctl(struct caen_ctl *cmd)
{
   ...
   if (cmd->cvt_num < 0 || cmd->cvt_num > 11) return(CAEN_ERR_NONEXIST);
   if (!dev792[cmd->cvt_num]) return(CAEN_ERR_NONEXIST);
   caen =  CAEN792_LIST[cmd->cvt_num];
   byte_swap_((char *)cmd->data,sizeof(cmd->data));
