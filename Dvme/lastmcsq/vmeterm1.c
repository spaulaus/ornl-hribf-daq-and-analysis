...
*/
   termio = open("/dev/ttyS1",O_RDWR);
   if (termio < 3)
     {
       perror("VMEterm");
       printf("Can not open serial line ttyS1\n");
       exit(99);
     }
   if (ioctl (termio,TCGETA,&init_tty) == -1)
     {
       perror("VMEterm");
       printf("Can not get ttyS1 configuration\n");
       close(termio);
       exit(99);
     }
/*
*   Configure ttyS1 for our use.
*/
   ...
   if (ioctl (termio,TCSETA,&vme_tty) == -1)
     {
       perror("VMEterm");
       printf("Can not set ttyS1 configuration\n");
       close(termio);
       exit(99);
     }
         ...
         if (cmd[0] == ';')
           {
             tbuf[0] = '\r';
             tbuf[1] = 0;
             if (!strcmp(cmd,";loadvme"))
               {
                 cptr = getfield(cmd,cptr,sizeof(loadname));
                 if (cptr != NULL)
                   {
                    strcpy(tbuf,"tftpGet -a4000000 -f");
                    strcat(tbuf,cmd);
                   }
                 else strcpy(tbuf,"tftpGet -a4000000 -fvmeacq.bin");
               }
             if (!strcmp(cmd,";go"))
               {
                 strcpy(tbuf,"go -a4000000");
               }
             len =strlen(tbuf);
             tbuf[len] = '\r';
             tbuf[len+1] = 0;
...
{
/*
*   Restore ttyS1 to the state it was when this program started
*/
...
static void helpmesg(void)
{
   ...
   printf("\n");
   printf(";loadvme file - Download the MVME5500\n");
   printf(";go           - Start the MVME5500\n");
   printf("\n");
