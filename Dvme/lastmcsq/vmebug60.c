     ...
     switch  ((enum func)i)
      {
        ...
        case  DE:        /* Select a VME device interface   */
          ...
          break;
        case  MM:        /* modify memory in CPU-60        */
          size = 2;
          if (command[3] == 'b') size = 1;
          else if (command[3] == 'l') size = 4;
...
void vmebug_help(void)
{
  ...
  "\n",
  "  dm.x  saddr eaddr  Display CPU-60 memory in hex and ASCII format.\n",
  "  di.x  saddr eaddr  Disassemble CPU-60 code.\n",
  "  mm.x  saddr eaddr  Modify memory in CPU-60. Format codes - x -\n",
  "                     are: B W L or C.\n",
  "  dev  device        Debug a hardware device. Device codes are:\n",
  "                     LRS KSC ACRO TRIG CES FERA1 FERA2 FERA3 FERA4.\n",
  "  dr   [n]           Display 68060 registers for task 'n' in CPU-60.\n",
  "  pac                Disassemble a PAC code\n",
  "  lc   [n]           Set output line counter.\n",
  "  lt                 Display tasks loaded in the CPU-60.\n",
...
/*****************************************************************************
*
*   Display Force CPU-60 memory in hexadecimal format.
*
...
*/
   ...
   lan_printf("CPU-60 memory - hex/ASCII display\n");
...
/*****************************************************************************
*
*   Modify Force CPU-60 memory.  NOTE: This is restricted to "data".
*   Modification of "code" may or may not work since the 68060 has separate
...
void  memory_modify(int size,int args)
{
   ...
   lan_printf("CPU-60 memory modify\n");
...
*/
     ...
     if (args == 1)
      {
/*
*   If user modified the location, make the change in CPU-60 memory.
*/
       switch  (size)
        {
          case  1:
...
*/
   ...
   lan_printf("\nBus Error in %s\n",mes[i]);
   lan_printf("PC = %x, Fault Addr = %x, SR = %4.4x, Special Status = %8.8x\n",
                                                                 pc,ea,sr,ssw);
}
#ifdef  OPT
/****************************************************************************
