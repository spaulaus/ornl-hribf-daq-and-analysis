...
/*
*   Pointers to CAEN ADCs TDCs QDCs
*/
struct CAEN *CAEN785_LIST[12] = {       /* V785 ADC             */
...
struct CAEN *CAEN775_LIST[12] = {       /* V775 TDC             */
...
unsigned short  Caen_adc_id[408];
unsigned short  Caen_tdc_id[408];
unsigned short  Caen_qdc_id[408];

int vme_enable;
int vme_delay;
struct vme_readout caen_775;
struct vme_readout caen_785;
struct vme_readout caen_792;
...
struct caen_readout caen_adcs[13]; /* 23May08 add for Bardayan */
struct caen_readout caen_tdcs[13];
struct caen_readout caen_qdcs[13];
...
#ifdef DEBUG
   host_message(INFORM,"CAEN ADC Table **************");
#endif
...
int init_vme_modules(void)
{
  ...
  struct cond_vme_pgm *prog;
   ...
   for (i=0; i < 13; i++) caen_adcs[i].hwd = NULL;
   for (i=0; i < 13; i++) caen_tdcs[i].hwd = NULL;
   for (i=0; i < 13; i++) caen_qdcs[i].hwd = NULL;
       ...
       switch (prog->type_code)
        {
          ...
          case  CAEN_792:
            ...
            break;
          default:
            sprintf(error_msg,"While looking at offset %i, there were %i items",
                    tblptr->offset, count);
            host_message(PANIC,error_msg);
            sprintf(error_msg,"Unknown VME module type %i in readout specs",prog->type_code);
            host_message(PANIC,error_msg);
            ++prog;
            --count;
            continue;
