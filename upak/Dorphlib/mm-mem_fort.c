...
void mmap_hisspace__(int *fd, int *protflag, int *ierr){
  ...
  */
  ...
  ret = fstat(*fd, &sb);
  fsize =  sb.st_size;
  /*printf("File status: retstatus=%i, size=%i, compare with %i\n", 
	 ret, fsize, *size);
  */
...
*/
void setmemory__(void *addr, int *value, int *bytes)
{
    size_t n;
    int c;
    c = *value;
...
/*  SUBROUTINE COPYMEMORY(DEST,SOURCE,NBYTES)
    set nbytes memory block to value of low order byte of value.
*/
void copymemory__(void *s2, void *s1, int *bytes){
    size_t n;
    n = *bytes;
    memcpy(s2,s1,n);
}
