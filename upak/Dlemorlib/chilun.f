...
C     INTEGER*4  NPR  - Number of parameters in this event.
...
C
C     NPR  = No. of parameters stored in EVBUF
...
C
      INTEGER*4 EVBUF(*)
C
      INTEGER*4 NHW,NPR,MXPAR
C
      INTEGER*4 DATA,I,J,ID,BNDX
C
      CHARACTER*4  STAT
C
      DATA BNDX /1/
...
C                                      !Otherwise, we have one
      NPR = NPR + 1                    !parameter extraction completed
      EVBUF(ID) = IAND(DATA,Z'FFFF')   !Store DATA
...
C
      IF(NPR.GT.0) THEN             !Check for any data loaded
