...
C     ------------------------------------------------------------------
      INTEGER*4    IERR,JERR
C
      INTEGER*4    IV,JV,NPARU,KOUSIZ,KS
C
...
C
      IF(KMD.EQ.'CLID') GO TO 1850
C
      RETURN
C
  140 CALL HEDMAN(IERR)
...
C     ------------------------------------------------------------------
C
 1850 CALL IVALU(LWD(1,2),IV,JERR)
      IF(JERR.NE.0)              GO TO 4000
      IF(IV.LT.0.OR.IV.GE.32768) GO TO 4000
      CALL IVALU(LWD(1,3),JV,JERR)
      IF(JERR.NE.0)              GO TO 4000
      IF(JV.LT.0.OR.JV.GE.32768) GO TO 4000
      CLIDHI=IV
      CLIDLO=JV
      GO TO 5500
C
