...
C
C     ------------------------------------------------------------------
C     COPY N (FILES)  OR  CREC N (RECORDS) FROM INPUT TO OUTPUT-1
C     ------------------------------------------------------------------
C
      IERR=0
      MSGX=0
...
C
      IF(LINO.NE.'YES '.AND.KINPUT.NE.'FILE') GO TO 1020
...
C
  500 IF(MSGF.NE.'    ') THEN
                         ...
                         ENDIF
C
      CALL READUM1(LUCI,IBUF,65535,NBYT,STAT)
...
C
  ...
  510 FORMAT(15A4,' - #',I12)
C
  520 CALL WRITUM(LUCO(1),IBUF,NBYT,STAT)
      LRIT='DATA'                          !SET LAST-RIT-FLG='DATA'
...
C
 ...
 1035 FORMAT('OUTPUT TAPE/FILE NOT ASSIGNED - COMMAND IGNORED')
C
