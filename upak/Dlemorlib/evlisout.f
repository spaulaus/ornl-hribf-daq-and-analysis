C$PROG EVLISOUT  - Outputs 1 event-list to stream-1 (UPON-mode)
C
C     ******************************************************************
C     BY W.T. MILNER AT HRIBF - LAST MODIFIED 04/23/2002
C     ******************************************************************
C
      SUBROUTINE EVLISOUT(IDBUF,DABUF,NID)
C
      IMPLICIT NONE
C
C     ------------------------------------------------------------------
...
C     ------------------------------------------------------------------
      INTEGER*4    IDBUF(*),DABUF(*),NID
C
      INTEGER*4    ILO,IHI,NWDS,OOF,OEND,PARMID,LASID,ID,I,J
C
      INTEGER*4    X8000
      DATA         X8000/Z'8000'/
C
...
C
      ...
      ID=IDBUF(I)                          !ID#
      IF(ID.NE.LASID+1) THEN               !Tst for non-contig IDs
      PARMID=ID+X8000                      !Parm-ID
      OUBUF(OOF)=IAND(PARMID,Z'FFFF')      !Load output with ID
      OOF=OOF+1                            !Inc  output pointer
      ENDIF
C
      LASID=ID                             !Save last ID
      OUBUF(OOF)=DABUF(I)                  !Load output with DATA
      OOF=OOF+1                            !Inc  output pointer
  220 CONTINUE
      OUBUF(OOF)=-1                        !Add the mighty FFFF
