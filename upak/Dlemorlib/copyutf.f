C$PROG COPYUTF   - Copies an udf-file to an ldf-file
C
C     ******************************************************************
C     BY W.T. MILNER AT HRIBF - LAST MODIFIED 04/23/2002
C     ******************************************************************
C
      SUBROUTINE COPYUTF(KMD,NFE,IERR)
C
      IMPLICIT NONE
C
C     ------------------------------------------------------------------
...
C
      WRITE(6,777)
  777 FORMAT('Got past LDFOOK')
C
      MSGX=0
      LRIT='DATA'                          !SET LAST-RIT-FLG='DATA'
C
      IF(INTYP.NE.'UDF ')    GO TO 1020
C
...
C
      INRECI=INRECI+1
C
CX    IF(INRECI.GT.MXRECI) RETURN
C
      CALL UDFREAD(JBUF,KIND,NBYT,IERR)
C
      IF(IERR.EQ.0) STAT='GOOD'
C
      IF(IERR.NE.0) STAT='BAD '
C
