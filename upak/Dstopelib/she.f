C$PROG SHE       - S(ELE) for heavy-ions above 1.5 MeV - Ziegler's
C   
C     ******************************************************************
C     By W.T. Milner at HRIBF - last modified 04/17/2000
C     ******************************************************************
C
      FUNCTION SHE(A1,Z1,A2,Z2,E)
C
      DIMENSION XMIP(92)
C
      DATA (XMIP(I),I=1,92)/
...
C
      CONZ2=SHELCO(E,Z2)
      EFC=Z1*HICRAT(A1,Z1,E)
      GSQ=1.0/(1.0+E/(931478*A1))**2
      BSQ=1.0-GSQ
      FOB=ALOG(2.0*XMC2*BSQ/GSQ)-BSQ
      IZ=Z2+0.5
      ALI=ALOG(XMIP(IZ))
      SHE=4.0*PI*EFC*EFC*Z2*ETO4*(FOB-ALI-CONZ2)/(BSQ*XMC2)
      RETURN
      END
