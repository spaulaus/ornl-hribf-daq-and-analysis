...
C     ------------------------------------------------------------------
      ...
      CHARACTER*4  KSORB
C     ------------------------------------------------------------------
      REAL*4       ENER(1000),ZIEG(1000),WARZ(1000),NORT(1000)
C
      REAL*4       ELST(40)
C
...
C
      CTILS=' '
      IF(KINTARG.EQ.'    ') WRITE(CTILS,205)AP,ZP,AT,ZT
      IF(KINTARG.NE.'    ') WRITE(CTILS,210)AP,ZP,KINTARG
C
...
C
      ...
      KINDE=1
C
      IF(EA.GT.0.0.AND.EB.GT.0.0.AND.DE.GT.0.0) GO TO 240
C
      KINDE=2
      NE=37
      NLO=1
...
C
  300 CONTINUE
C
      CALL PLOTDEDX(ENER,WARZ,ZIEG,NORT,NVAL)
C
