...
C     ------------------------------------------------------------------
C
  300 IF(NF.GE.2) GO TO 305
C
      IF(LASIDN.LE.0) GO TO 2000
      IDN=LASIDN
      GO TO 315
C
  305 CALL MILV(LWD(1,2),IDN,SV,KIND,IERR)
      IF(IERR.NE.0) GO TO 2010
C
...
C
  ...
  320 CONTINUE
C
  325 IF(KMD.GE.'GOMO'.AND.LASIDN.LE.0) GO TO 2000
C
      CALL WTCOMP
C
      IF(KMD.EQ.'GOMO') THEN
...
C
  ...
  820 CONTINUE
      NLIN=101
C
      CALL PLOTFIT(2,XIN,YIN,YCALSAV,NDAT,XLIN,YLIN,NLIN)
C
      GO TO 2500
C
C     ------------------------------------------------------------------
...
C
 ...
 2005 FORMAT('Data ID number not defined - command not processed')
      GO TO 2400
C
