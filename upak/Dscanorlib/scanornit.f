...
C
      ...
      character*4  cIHUP,cIGET,cIZOT
      equivalence  (cIHUP,IHUP), (cIGET,IGET),(cIZOT,IZOT)
      DATA         cIHUP,cIGET,cIZOT/'HUP ','GET ','ZOT '/
C
...
C
      ...
      DATA        cNAMPROG/'SCAN','MO  '/
C
      INTEGER*4    BLANK,CON
      character*4  cblank, ccon
      equivalence  (cblank,blank), (ccon, con)
      DATA         cBLANK,cCON/'    ','CON:'/
C
C
      integer*4    hisfilesize
      integer*4    fsize     ! stat information 
      integer*4    status
...
C     ------------------------------------------------------------------
c     MEMORY-MAPPED I/O - This routine is modified for testing memory
c     mapped I/O
C  
      NBYTHIS=2*NHWH
C
      IF(LUC(7).EQ.555)THEN     !If HIS file new (for VM version)
         LUC(7)=LUC(6)
Cmm   A new file must be written before mapping to get the file size.
         CALL zero_hisfile(nbythis,LUH,ierr)
         if(ierr .ne. 0) then
            return
         endif
      else
         fsize=hisfilesize(LUH)
         if (fsize .eq. -1) then
            write(logut,*) 'Unable to fstat existing .his file'
            return
         endif
         if (nbythis .gt. fsize) then          !file size in bytes
*         Extend the file by writing zeros
            write(6,*) 'Extending with zeros'
            CALL zero_hisfile(nbythis,LUH,ierr)
            if(ierr .ne. 0) then
               return
            endif
      ...
      ENDIF
C
cmm   Memory map the histogram at this point
      call mmap_hisspace(LUH, 1, ierr)
C
      IF(IERR.EQ.0)THEN
