C$PROG UDFREAD   - Reads UDF-file and loads IBUF - for SCANOR
C
C     ******************************************************************
...
C
C     IBUF structure can be either L001, L002 or L003.
C
C     L001 is the most simple if you can use it. It has a fixed no. of
C          parameters per event and no parameter ID's.
C          You must specify L001 NSKIP NPPE in SCANOR where:
C          NSKIP is the number of leading words to skip - probably 0
C          NPPE  is the number of parameters per event
C          (see Handbook U100.090 & U320.030)
C     ------------------------------------------------------------------
C
      IERR=0
C
