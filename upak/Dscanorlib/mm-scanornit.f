...
C
C
      integer*4    sarray(13)     ! stat information 
      integer*4    status
...
*      ENDIF
      ...
      IF(LUC(7).EQ.555)THEN     !If HIS file new (for VM version)
         LUC(7)=LUC(6)
Cmm   A new file must be written before mapping.
         CALL zero_hisfile(nbythis,LUH,ierr)
         if(ierr .ne. 0) then
            return
         endif
      else
         status=fstat(LUH, sarray)
         if (status .ne. 0) then
            write(logut,*) 'Unable to fstat existing .his file'
            return
         endif
         if (nbythis .gt. sarray(8)) then  !file size in bytes
            CALL zero_hisfile(nbythis,LUH,ierr)
            if(ierr .ne. 0) then
               return
            endif
