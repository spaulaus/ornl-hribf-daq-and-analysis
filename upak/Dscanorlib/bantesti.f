C$PROG BANTESTI  - Tests a free-form-gate by ID-number
C
C     ******************************************************************
...
C
      EXTERNAL     ISHFT
C
      INTEGER*4    IG,IX,IY,NG,IDX,ISHFT
C
      INTEGER*2    JY
C
C     ------------------------------------------------------------------
...
C     ------------------------------------------------------------------
      ...
      BANTESTI=.FALSE.
C
      IF(IG.GT.MAXID)    RETURN  !Test for BAN-ID out-of-range
C
      IF(IG.LT.1)        RETURN  !Test for BAN-ID out-of-range
C
      NG=IDBAN(IG)               !Get Gate-number
C
...
C
      IF((JY.GE.LFGL(IDX).AND.JY.LE.LFGH(IDX))) BANTESTI=.TRUE.
C
      RETURN
C
      END
